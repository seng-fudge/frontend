...
export default function PaymentForm({ setIndex }) {
  const { token, payment, setPayment } = useContext(UserContext);

  useEffect(() => {
    const values = document.getElementById("paymentForm");

    if (payment != null) {
      values.elements["formDueDate"].value = payment["dueDate"];
      values.elements["formPaymentType"].value = payment["paymentType"];
      values.elements["formPaymentId"].value = payment["paymentId"];
      values.elements["formPaymentTerms"].value = payment["paymentTerms"];
    }
  }, [payment]);
  ...
  return (
    <form id="paymentForm" onSubmit={() => onSubmit()}>
      ...
      <FormInput id="formPaymentId" name="Payment Id" type="text" />
